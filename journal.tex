...
\begin{minipage}[b]{0.3\linewidth}
	...
	\caption{Power consumption for server and router under different server bandwidth allocation strategies when the peer arrival rate of less popular files varies. We use server bandwidth $S=50$MBps.}
...
\begin{minipage}[b]{0.3\linewidth}
	...
	\caption{Total power consumption under different server bandwidth allocation strategies when the peer arrival rate of less popular files varies. We use server bandwidth $S=50$MBps.}
	\label{fig:totalpop}
\end{minipage}
...
We examine popularity by varying the peer arrival rate of less popular files while server has fix bandwidth.
...
When file popularity is low there are fewer peers downloading the file and hence each of them can obtain relatively more server bandwidth.
...
Figure \ref{fig:componentpop} shows that CDN server energy consumption is flat because we use fix server bandwidth to $S=50$MBps and router energy consumption is increasing due to increasing number of peers.  
...
Guan et al. \cite{5963557} comparing energy efficient of CDN architecture and CCN architecture.
